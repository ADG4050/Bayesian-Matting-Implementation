...
def maximum_likelihhod(trimap):
    org = np.floor(trimap.astype(float)*255/(255*255))

    vres, hres, channels = org.shape
    numpels = hres * vres

    y, u, v = rgb2yuv(org[:,:,0], org[:,:,1], org[:,:,2])

    sx = slice(200, 600)
    sy = slice(400, 650)
    u_bar = np.mean(u[sy, sx])
    u_var = 2 * (np.std(u[sy, sx]))**2

    v_bar = np.mean(v[sy, sx])
    v_var = 2 * (np.std(v[sy, sx]))**2

    y_bar = np.mean(y[sy, sx])
    y_var = 2 * (np.std(y[sy, sx]))**2
...
def main():
    ...
    img = cv2.imread("C:/Users/aduttagu/Desktop/Bayesian-Matting-Implementation/input_training_lowres/GT04.png")[:, :, :3]
    trimap = cv2.imread("C:/Users/aduttagu/Desktop/Bayesian-Matting-Implementation/trimap_training_lowres/Trimap1/GT04.png")
    GT = cv2.imread("C:/Users/aduttagu/Desktop/Bayesian-Matting-Implementation/gt_training_lowres/GT04.png")
    trimap = trimap[:, :, 0]
    GT = GT[:, :, 0]
    ...
    alpha_ML = maximum_likelihhod(img)
    
    plt.figure(2)
    plt.imshow(alpha_ML * 255, cmap='gray')
    plt.title('ML')
    plt.show()
